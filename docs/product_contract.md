...
This file defines what the project delivers and which measurements decide
whether an artifact is acceptable. It is a target for later scientific
validation, not a model result.

## Calibrated ionogram artifact

One CSA film scan produces a quality-gated `isis.ionogram.v1` artifact with:
...
The contract is implemented in `src/isis_research/ionogram.py`.
`write` rejects malformed artifacts. `read_validated` rejects non-canonical
orientation, missing metadata or masks, invalid scale, and inconsistent axes.
The reader also understands historical layouts for audit and migration.

Film traces are dark, so signal-positive consumers use
`1 - warped_intensity`. Storage orientation remains `(height, frequency)`;
consumer-specific transposes happen only at their boundaries.

## Model-derived CDF-like output

The downstream file is a model-derived NASA-CDF-like amplitude file. Its axes,
validity mask, and signal come from the calibrated CSA artifact and model
output. Fields unavailable from a film scan are explicit unknowns. The file
can be compared with a measured NASA CDF, but it is not claimed to be the same
measurement. Physical electron-density profiles are outside the current scope.
...
therefore remain unmeasured until an expert-reviewed reference set exists.

## Current boundary

The current product is a calibrated digital ionogram plus a model-derived
amplitude file. It does not yet establish robust external generalization,
absolute-height accuracy, marker identity, or physical inversion. Those claims
require references beyond file compatibility and fit residuals.
