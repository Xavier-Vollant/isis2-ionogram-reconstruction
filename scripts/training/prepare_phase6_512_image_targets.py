#!/usr/bin/env python3
"""Prepare direct NASA image targets for the usable native CSA corpus.

The target is the matched NASA ``ampl`` image resampled onto each usable
CSA artifact's exact frequency/height axes.  No trace extraction or manual
labels are involved.  Amplitude is normalized to [0, 1], strong NASA signal
remains high, and ``valid_mask`` marks CDF regions that genuinely exist.
"""
...
def target_for(row, corpus):
    ...
    if target.shape != GRID_SHAPE or valid.shape != GRID_SHAPE:
        raise ValueError(f"{row['pair_name']}: target is {target.shape}, expected {GRID_SHAPE}")
    if not valid.any():
    ...
    normalized = np.clip(np.nan_to_num(target / 255.0, nan=0.0), 0.0, 1.0).astype(np.float32)
...
def main():
    ...
    for index, row in enumerate(rows, 1):
        target_path = args.output / "targets" / f"{Path(row['csa_artifact']).stem}_target.npz"
        ...
        if index == 1 or index % 25 == 0 or index == len(rows):
            print(f"prepared target {index}/{len(rows)}: {row['pair_name']}", flush=True)
    ...
    summary = {
        ...
        "mean_valid_fraction": float(np.mean([float(row["target_valid_fraction"]) for row in records])),
        "mean_target_std": float(np.mean([float(row["target_std"]) for row in records])),
    }
